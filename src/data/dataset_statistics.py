...
def dataset_statistics(df) -> Dict:
    ...
    statistics['#exclusions'] = len(df[df['labels'].astype(int) == 0])
    statistics['%exclusions'] = len(df[df['labels'].astype(int) == 0]) / len_df
    statistics['#inclusions'] = len(df[df['labels'].astype(int) == 1])
    statistics['%inclusions'] = len(df[df['labels'].astype(int) == 1]) / len_df
    ...
    statistics['avg_word_num_inclusions'] = inclusions['abstracts'].str.split().str.len().mean()
    statistics['avg_word_num_exclusions'] = exclusions['abstracts'].str.split().str.len().mean()
...
if __name__ == '__main__':
    ...
    for file in tqdm(os.listdir(data_dir)):
        ...
        total_statistics[file] = dataset_statistics(df=df)

    results = pd.DataFrame.from_dict(total_statistics).transpose()
    results.to_csv("../../data/statistics.csv")